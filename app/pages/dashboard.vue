...
async function clock(action: 'in' | 'out') { busy.value = true; error.value = ''; try { await $fetch('/api/attendance/clock', { method: 'POST', body: { action, ...(action === 'out' ? { recordId: today.value?.openRecord?.id } : {}) } }); await load() } catch (cause: any) { error.value = cause?.data?.statusMessage || '打刻に失敗しました。'; await load() } finally { busy.value = false } }
...
<template>
  <main class="app-shell">
    ...
    <section class="card clock-card">
      ...
      <div class="actions"><button class="primary" :disabled="busy || loading || !!today.record?.clock_in_at" @click="clock('in')">出勤を記録</button><button class="secondary" :disabled="busy || loading || !today.openRecord" @click="clock('out')">退勤を記録</button></div>
